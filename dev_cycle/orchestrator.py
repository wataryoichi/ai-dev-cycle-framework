...
def _auto_fill_final_summary(cycle_dir: Path) -> None:
    """Auto-generate final-summary.md from implementation summary + review."""
    from .cycle import _is_placeholder, _read_meta
    summary_path = cycle_dir / "final-summary.md"
    if not _is_placeholder(summary_path):
        return  # already filled

    meta = _read_meta(cycle_dir)
    lang = meta.get("lang", "en")

    # Gather content from implementation summary
    impl_path = cycle_dir / "claude-implementation-summary.md"
    impl_text = impl_path.read_text() if impl_path.exists() else ""
    # Extract "What Was Done" section
    import re
    done_match = re.search(r"## What Was Done\s*\n(.*?)(?=\n##|\Z)", impl_text, re.DOTALL)
    overview = done_match.group(1).strip() if done_match else meta.get("title", "")

    # Gather findings from review
    findings = count_findings(cycle_dir)
    changes = []
    if overview:
        for line in overview.splitlines():
            ...
            if line and not line.startswith("<!--"):
                changes.append(line.lstrip("- *0123456789."))

    remaining = []
    if findings["total"] > 0:
        remaining.append(f"{findings['total']} review finding(s) remaining")

    from .dual_output import write_final_summary
    write_final_summary(
        cycle_dir,
        overview=overview,
        changes=changes or [meta.get("title", "cycle")],
        verification="Auto-generated by devcycle turbo",
        remaining=remaining,
        lang=lang,
    )
...
def _generate_readme(cycle_dir: Path, project_root: Path) -> Path | None:
    ...
    # List generated files (non-meta, non-template files)
    artifacts = sorted(
        f.name for f in cycle_dir.iterdir()
        if f.is_file()
        and f.suffix in (".html", ".js", ".css", ".py", ".ts", ".json")
        and f.name not in ("meta.json", "cycle_state.json", "request.json",
                           "implementation_summary.json", "fix_plan.json",
                           "findings_diff.json")
    )
    if artifacts:
        lines.extend(["## Files", ""])
    ...
    lines.extend([
        "---",
        f"*Generated by [AI Dev Cycle Framework](https://github.com/wataryoichi/ai-dev-cycle-framework) v{meta.get('version', '')}*",
        "",
    ])
        ...
        if state == State.REVIEW_PENDING:
            ...
            if codex_result["success"] and codex_result["review_text"]:
                ...
                if fix_rounds > 0 and post_import_findings["total"] == 0:
                    ...
                    _auto_fill_final_summary(cycle_dir)
