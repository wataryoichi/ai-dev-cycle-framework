...
_TRANSITIONS: dict[State, list[Transition]] = {
    ...
    State.IMPLEMENTING: [
        Transition(
            ...
            choices=[
                Choice(1, "Implementation is done, proceed to review", "prepare_review"),
                Choice(2, "Not done yet, exit (resume later)", "exit"),
            ],
        ),
    ],
    State.REVIEW_NEEDED: [
    ...
    State.FOLLOWUP_READY: [
        Transition(
            ...
            choices=[
                Choice(1, "I've reviewed the followup, apply fixes with Claude", "apply_fixes"),
                Choice(2, "Exit to edit followup manually (resume later)", "exit"),
                Choice(3, "Skip fixes, finalize now", "skip_to_finalize"),
            ],
        ),
    ],
    State.FIX_NEEDED: [
        Transition(
            ...
            choices=[
                ...
                Choice(2, "Re-run Codex review", "rereview"),
                Choice(3, "Exit to continue fixing (resume later)", "exit"),
            ],
        ),
    ],
    State.READY_TO_FINALIZE: [
        Transition(
            ...
            "Finalize the cycle",
            choices=[
                Choice(1, "Finalize (strict — fails on issues)", "finalize_strict"),
                Choice(2, "Finalize (allow warnings)", "finalize"),
                Choice(3, "Re-run Codex review first", "rereview"),
            ],
        ),
    ],
}
